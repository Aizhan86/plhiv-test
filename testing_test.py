...
from pages.main_page import ArvRegisters
...
class TestMedicationIssuance():
    def test_receipt_register_for_pregnancy_modal(self, browser):
        login(browser)
        work_page = WorkPage(browser, browser.current_url)
        work_page.choose_Turkestan_area_as_user_org()
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.fill_receipt_register_for_pregnancy_modal()

    @pytest.mark.skipif(test_receipt_register_for_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_save_button_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_save_button_receipt_register()

    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_add_button_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_add_button_receipt_register()

    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_receipt_type_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.add_medication_for_adult_on_balance_register()
        main_page.check_receipt_type_receipt_register()

    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_budget_type_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_budget_type_receipt_register()

    @pytest.mark.xfail(reason="supplier doesn't take a value")
    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_supplier_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_supplier_receipt_register()

    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_waybill_number_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_waybill_number_receipt_register()

    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_invoice_number_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_invoice_number_receipt_register()

    @pytest.mark.xfail(reason="need to correct format")
    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_receipt_date_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_receipt_date_receipt_register()

    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_programm_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_programm_receipt_register()

    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_recipient_name_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_recipient_name_receipt_register()

    # @pytest.mark.xfail(reason="art_reasons_define_date object doesn't save its value")
    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_procurement_type_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_procurement_type_receipt_register()

    # @pytest.mark.xfail(reason="art_readiness_define_date object doesn't save its value")
    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_contract_number_date_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_contract_number_date_receipt_register()

    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_expiration_date_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_expiration_date_receipt_register()

    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_amount_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_amount_receipt_register()

    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_price_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_price_receipt_register()

    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_series_number_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_series_number_receipt_register()

    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_sum_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_sum_receipt_register()

    @pytest.mark.skipif(test_save_button_receipt_register == "FAILED", reason="patient id wasn't taken")
    def test_cancel_button_receipt_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_cancel_button_receipt_register()

    def test_receipt_register_for_child_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.fill_receipt_register_for_child_in_pregnancy_modal()
        main_page.add_medication_for_child_on_balance_register()

    def test_receipt_register_for_art_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.fill_receipt_register_for_art_modal()
        main_page.add_medication_for_art_adult_on_balance_register()

    def test_write_off_register_for_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.fill_write_off_register_for_pregnancy_modal()

    @pytest.mark.skipif(test_write_off_register_for_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_save_button_write_off_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_save_button_write_off_register()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_add_button_write_off_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_add_button_write_off_register()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_waybill_number_write_off_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.swift_on_editing_page_for_adult_write_off_register()
        main_page.check_waybill_number_write_off_register()

    @pytest.mark.xfail(reason="need to correct format")
    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_waybill_date_write_off_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_waybill_date_write_off_register()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_sender_name_write_off_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_sender_name_write_off_register()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_budget_type_write_off_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_budget_type_write_off_register()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_programm_write_off_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_programm_write_off_register()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_recipient_write_off_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_recipient_write_off_register()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_recipient_name_write_off_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_recipient_name_write_off_register()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_write_off_reason_write_off_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_write_off_reason_write_off_register()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_amount_write_off_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_amount_write_off_register()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_sum_write_off_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_sum_write_off_register()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_cancel_button_write_off_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_cancel_button_write_off_register()

    def test_write_off_register_for_child_in_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.fill_write_off_register_for_child_in_pregnancy_modal()
        main_page.check_save_button_write_off_register()
        main_page.swift_on_editing_page_for_child_write_off_register()

    def test_write_off_register_for_adult_art_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.fill_write_off_register_for_art_modal()
        main_page.check_save_button_write_off_register()
        main_page.swift_on_editing_page_for_art_adult_write_off_register()

    # @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_receipt_medication_for_adult_from_Turkestan(self, browser):
        work_page = WorkPage(browser, browser.current_url)
        work_page.choose_Zhetysai_as_user_org()
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.receipt_medicine_for_adult()

    def test_receipt_medication_for_art_adult_from_Turkestan(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.receipt_medicine_for_art_adult()

    def test_receipt_medication_for_child_from_Turkestan(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.receipt_medicine_for_child()

    # @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_medicine_in_balance_register(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_medicine_in_balance_register()

    @pytest.mark.smoke
    def test_registration_of_woman(self, browser):
        # login(browser)
        work_page = WorkPage(browser, browser.current_url)
        work_page.open_main_page()
    ...
    def test_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.fill_pregnancy_modal()
    ...
    def test_children_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.fill_children_modal()

    @pytest.mark.skipif(test_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_save_button_children_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_save_button_children_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_add_button_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_add_button_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_edit_button_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_edit_button_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_pregnancy_in_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_pregnancy_in_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_sex_partner_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_sex_partner_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_hiv_status_of_partner_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_hiv_status_of_partner_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_ogc_registration_date_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_ogc_registration_date_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_ogc_pregnancy_weeks_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_ogc_pregnancy_weeks_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_antenatal_clinic_registration_date_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_antenatal_clinic_registration_date_pregnancy_modal()

    def test_wc_pregnancy_weeks_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_wc_pregnancy_weeks_pregnancy_modal()

    def test_pregnancy_result_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_pregnancy_result_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_arv_prophylaxys_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_arv_prophylaxys_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_arv_prophylaxys_start_date_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_arv_prophylaxys_start_date_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_pregnancy_weeks_arv_start_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_pregnancy_weeks_arv_start_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_arv_prophylaxys_end_date_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_arv_prophylaxys_end_date_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_pregnancy_weeks_arv_end_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_pregnancy_weeks_arv_end_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_arv_medication_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_arv_medication_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_medication_issuance_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_medication_issuance_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_medication_name_medication_issuance_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_medication_name_medication_issuance_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_add_button_medication_issuance_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_add_button_medication_issuance_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_edit_button_medication_issuance_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_edit_button_medication_issuance_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_cancel_button_medication_issuance_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_cancel_button_medication_issuance_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_medication_issuance_for_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_medication_issuance_for_child_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_medication_name_medication_issuance_for_future_child_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_medication_name_medication_issuance_for_future_child_modal()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_cancel_button_medication_issuance_for_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_cancel_button_medication_issuance_for_child_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_write_off_register == "FAILED", reason="patient id wasn't taken")
    def test_edit_button_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_edit_button_child_pregnancy_modal()

    def test_alive_child_in_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_alive_child_in_child_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_childs_surname_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_childs_surname_child_pregnancy_modal()

    def test_childs_name_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_childs_name_child_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def check_childs_middle_name_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_childs_middle_name_child_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_childs_birth_date_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_childs_birth_date_child_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_childs_gender_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_childs_gender_child_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_pathalogy_at_birth_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_pathalogy_at_birth_child_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_childs_feeding_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_childs_feeding_child_pregnancy_modal()

    def test_full_term_child_in_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_full_term_child_in_child_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_childs_death_date_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_childs_death_date_child_pregnancy_modal()

    def test_cancel_button_child_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_cancel_button_child_pregnancy_modal()

    @pytest.mark.skipif(test_save_button_children_pregnancy_modal == "FAILED", reason="patient id wasn't taken")
    def test_cancel_button_pregnancy_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_cancel_button_pregnancy_modal()
    ...
    #
    # @pytest.mark.skipif(test_perinatal_registration_modal == "FAILED", reason="patient id wasn't taken")
    ...
    #
    # @pytest.mark.skipif(test_save_perinatal_registration_modal == "FAILED", reason="patient id wasn't taken")
    # def test_perinatal_registration_tab(self, browser):
    #     main_page = ArvRegisters(browser, browser.current_url)
    #     main_page.fill_perinatal_registration_tab()
    #
    # @pytest.mark.skipif(test_perinatal_registration_tab == "FAILED", reason="patient id wasn't taken")
    # def test_arv_prophylaxis_perinatal_registration_tab(self, browser):
    #     main_page = ArvRegisters(browser, browser.current_url)
    #     main_page.check_arv_prophylaxis_perinatal_registration_tab()
    #
    # @pytest.mark.skipif(test_perinatal_registration_tab == "FAILED", reason="patient id wasn't taken")
    # def test_arv_start_date_perinatal_registration_tab(self, browser):
    #     main_page = ArvRegisters(browser, browser.current_url)
    #     main_page.check_arv_start_date_perinatal_registration_tab()
    #
    # @pytest.mark.skipif(test_perinatal_registration_tab == "FAILED", reason="patient id wasn't taken")
    # def test_arv_medication_perinatal_registration_tab(self, browser):
    #     main_page = ArvRegisters(browser, browser.current_url)
    #     main_page.check_arv_medication_perinatal_registration_tab()
    #
    # @pytest.mark.skipif(test_perinatal_registration_tab == "FAILED", reason="patient id wasn't taken")
    # def test_medication_issuanse_perinatal_registration_tab(self, browser):
    #     main_page = ArvRegisters(browser, browser.current_url)
    #     main_page.check_medication_issuanse_perinatal_registration_tab()
    #
    # @pytest.mark.skipif(test_perinatal_registration_tab == "FAILED", reason="patient id wasn't taken")
    # def test_medication_name_in_medication_issuanse_registration_tab(self, browser):
    #     main_page = ArvRegisters(browser, browser.current_url)
    #     main_page.check_medication_name_in_medication_issuanse_registration_tab()
    #
    # @pytest.mark.skipif(test_perinatal_registration_tab == "FAILED", reason="patient id wasn't taken")
    # def test_cancel_button_in_medication_issuanse_perinatal_registration_tab(self, browser):
    #     main_page = ArvRegisters(browser, browser.current_url)
    #     main_page.check_cancel_button_in_medication_issuanse_perinatal_registration_tab()
    #
    # @pytest.mark.skipif(test_perinatal_registration_tab == "FAILED", reason="patient id wasn't taken")
    # def test_pcp_prophylaxis_start_date_perinatal_registration_tab(self, browser):
    #     main_page = ArvRegisters(browser, browser.current_url)
    #     main_page.check_pcp_prophylaxis_start_date_perinatal_registration_tab()
    #
    # @pytest.mark.skipif(test_perinatal_registration_tab == "FAILED", reason="patient id wasn't taken")
    # def test_pcp_prophylaxis_end_date_perinatal_registration_tab(self, browser):
    #     main_page = ArvRegisters(browser, browser.current_url)
    #     main_page.check_pcp_prophylaxis_end_date_perinatal_registration_tab()
    #
    # @pytest.mark.skipif(test_perinatal_registration_tab == "FAILED", reason="patient id wasn't taken")
    # def test_hiv_status_perinatal_registration_tab(self, browser):
    #     main_page = ArvRegisters(browser, browser.current_url)
    #     main_page.check_hiv_status_perinatal_registration_tab()
    #
    # @pytest.mark.skipif(test_perinatal_registration_tab == "FAILED", reason="patient id wasn't taken")
    # def test_hiv_determination_date_perinatal_registration_tab(self, browser):
    #     main_page = ArvRegisters(browser, browser.current_url)
    #     main_page.check_hiv_determination_date_perinatal_registration_tab()

    @pytest.mark.skipif(test_registration_of_woman == "FAILED", reason="patient id wasn't taken")
    def test_art_information_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.fill_art_information_modal()

    @pytest.mark.skipif(test_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_save_button_art_information_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_save_button_art_information_modal()

    @pytest.mark.skipif(test_save_button_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_add_button_art_information_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_add_button_art_information_modal()

    @pytest.mark.skipif(test_save_button_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_edit_button_art_information_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_edit_button_art_information_modal()

    @pytest.mark.skipif(test_save_button_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_start_date_art_information_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_start_date_art_information_modal()

    @pytest.mark.skipif(test_save_button_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_row_art_information_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_row_art_information_modal()

    @pytest.mark.skipif(test_save_button_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_scheme_art_information_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_scheme_art_information_modal()

    @pytest.mark.skipif(test_save_button_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_medical_organization_art_information_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_medical_organization_art_information_modal()

    @pytest.mark.skipif(test_save_button_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_edit_button_art_issuance_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_edit_button_art_issuance_modal()

    @pytest.mark.skipif(test_save_button_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_medication_name_art_issuance_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_medication_name_art_issuance_modal()

    @pytest.mark.skipif(test_save_button_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_scheme_change_date_art_information_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_scheme_change_date_art_information_modal()

    @pytest.mark.skipif(test_save_button_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_scheme_change_type_art_information_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_scheme_change_type_art_information_modal()

    @pytest.mark.skipif(test_save_button_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_scheme_change_reason_art_information_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_scheme_change_reason_art_information_modal()

    @pytest.mark.skipif(test_save_button_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_cancel_button_art_information_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_cancel_button_art_information_modal()

    @pytest.mark.skipif(test_registration_of_woman == "FAILED", reason="patient id wasn't taken")
    def test_art_adherence_level_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.fill_art_adherence_level_modal()

    @pytest.mark.skipif(test_art_adherence_level_modal == "FAILED", reason="patient id wasn't taken")
    def test_save_button_art_adherence_level_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_save_button_art_adherence_level_modal()

    @pytest.mark.skipif(test_save_button_art_adherence_level_modal == "FAILED", reason="patient id wasn't taken")
    def test_add_button_art_adherence_level_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_add_button_art_adherence_level_modal()

    @pytest.mark.skipif(test_save_button_art_adherence_level_modal == "FAILED", reason="patient id wasn't taken")
    def test_edit_button_art_adherence_level_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_edit_button_art_adherence_level_modal()

    @pytest.mark.skipif(test_save_button_art_adherence_level_modal == "FAILED", reason="patient id wasn't taken")
    def test_year_art_adherence_level_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_year_art_adherence_level_modal()

    @pytest.mark.skipif(test_save_button_art_adherence_level_modal == "FAILED", reason="patient id wasn't taken")
    def test_quater_art_adherence_level_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_quater_art_adherence_level_modal()

    @pytest.mark.skipif(test_save_button_art_adherence_level_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_adherence_art_adherence_level_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_adherence_art_adherence_level_modal()

    @pytest.mark.skipif(test_save_button_art_adherence_level_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_adherence_low_reasons_art_adherence_level_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_adherence_low_reasons_art_adherence_level_modal()

    @pytest.mark.skipif(test_save_button_art_adherence_level_modal == "FAILED", reason="patient id wasn't taken")
    def test_side_effects_art_adherence_level_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_side_effects_art_adherence_level_modal()

    @pytest.mark.skipif(test_save_button_art_adherence_level_modal == "FAILED", reason="patient id wasn't taken")
    def test_cancel_button_art_adherence_level_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_cancel_button_art_adherence_level_modal()
    ...
    @pytest.mark.xfail(reason="art_readiness_define_date object doesn't save its value")
    @pytest.mark.skipif(test_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_reasons_define_date_art_tab(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_reasons_define_date_art_tab()

    @pytest.mark.xfail(reason="art_readiness_define_date object doesn't save its value")
    @pytest.mark.skipif(test_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_readiness_define_date_art_tab(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_readiness_define_date_art_tab()

    @pytest.mark.skipif(test_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_written_consent_art_tab(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_written_consent_art_tab()

    @pytest.mark.skipif(test_art_information_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_adherence_support_art_tab(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_adherence_support_art_tab()

    @pytest.mark.skipif(test_registration_of_woman == "FAILED", reason="patient id wasn't taken")
    def test_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.fill_recipe_modal()

    @pytest.mark.skipif(test_recipe_modal == "FAILED", reason="patient id wasn't taken")
    def test_save_button_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_save_button_recipe_modal()
    ...
    def test_add_button_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_add_button_recipe_modal()

    @pytest.mark.skipif(test_save_button_recipe_modal == "FAILED", reason="patient id wasn't taken")
    def test_edit_button_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_edit_button_recipe_modal()

    @pytest.mark.xfail(reason="waiting for schema of medication to be deleted")
    @pytest.mark.skipif(test_save_button_recipe_modal == "FAILED", reason="patient id wasn't taken")
    def test_recipe_number_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_recipe_number_recipe_modal()

    @pytest.mark.xfail(reason="waiting for schema of medication to be deleted")
    @pytest.mark.skipif(test_save_button_recipe_modal == "FAILED", reason="patient id wasn't taken")
    def test_recipe_date_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_recipe_date_recipe_modal()

    @pytest.mark.xfail(reason="waiting for schema of medication to be deleted")
    @pytest.mark.skipif(test_save_button_recipe_modal == "FAILED", reason="patient id wasn't taken")
    def test_art_medication_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_art_medication_recipe_modal()

    @pytest.mark.xfail(reason="waiting for schema of medication to be deleted")
    @pytest.mark.skipif(test_save_button_recipe_modal == "FAILED", reason="patient id wasn't taken")
    def test_dose_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_dose_recipe_modal()

    @pytest.mark.xfail(reason="waiting for schema of medication to be deleted")
    @pytest.mark.skipif(test_save_button_recipe_modal == "FAILED", reason="patient id wasn't taken")
    def test_dose_code_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_dose_code_recipe_modal()

    @pytest.mark.xfail(reason="waiting for schema of medication to be deleted")
    @pytest.mark.skipif(test_save_button_recipe_modal == "FAILED", reason="patient id wasn't taken")
    def test_concentration_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_concentration_recipe_modal()

    @pytest.mark.xfail(reason="waiting for schema of medication to be deleted")
    @pytest.mark.skipif(test_save_button_recipe_modal == "FAILED", reason="patient id wasn't taken")
    def test_packing_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_packing_recipe_modal()

    @pytest.mark.xfail(reason="waiting for schema of medication to be deleted")
    @pytest.mark.skipif(test_save_button_recipe_modal == "FAILED", reason="patient id wasn't taken")
    def test_quatity_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_quatity_recipe_modal()

    @pytest.mark.xfail(reason="waiting for schema of medication to be deleted")
    @pytest.mark.skipif(test_save_button_recipe_modal == "FAILED", reason="patient id wasn't taken")
    def test_unpacking_signs_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_unpacking_signs_recipe_modal()

    @pytest.mark.xfail(reason="waiting for schema of medication to be deleted")
    @pytest.mark.skipif(test_save_button_recipe_modal == "FAILED", reason="patient id wasn't taken")
    def test_signiture_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_signiture_recipe_modal()

    @pytest.mark.xfail(reason="waiting for schema of medication to be deleted")
    @pytest.mark.skipif(test_save_button_recipe_modal == "FAILED", reason="patient id wasn't taken")
    def test_cancel_button_recipe_modal(self, browser):
        main_page = ArvRegisters(browser, browser.current_url)
        main_page.check_cancel_button_recipe_modal()
